  $(document).on('keyup' , '.p-units, .r-units' , function(){
    ...
    if($(this).val() != ''){
        ...
        $(cTR).find('.available_stock').find('input').attr('name' , 'remaining_stock[]');
        
        var available_stock = parseFloat($(cTR).find('.available_stock').find('input').val());
        ...
        $(cTR).find('.new-stock').val(available_stock + units);
  ...
  $('.r-amount, .discount').on('keyup' , function(){
    if($(this).val() == ''){
      var re_amount = 0;
      $(this).val('0');
    }
    else{
      var re_amount = parseFloat($('#r-amount').val());
      var discount = $('.discount').val();
    }
    var total_amount = parseFloat($('.t-amount').val());
    if(total_amount > re_amount || total_amount > discount){
     $('.advance-amount').val("0");
      $('.amount-left').val(total_amount - re_amount - discount);
